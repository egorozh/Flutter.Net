...
- Continued `StyleFrom(...)` parity with Flutter defaults:
  ...
  - `ButtonStyle.Merge(...)` now composes state-property resolvers with null-fallback semantics (higher-priority style falls back to lower-priority/default style per state), matching Flutter `ButtonStyleButton` effective-value layering for disabled-state defaults,
  ...
  - added/updated regression coverage in `TextButton_StyleFrom_ForegroundColor_DerivesOverlayAndSplash`, `TextButton_StyleFrom_TransparentOverlay_DisablesVisualHighlights`, `TextButton_StyleFrom_OverlayColor_UsesStateOpacitiesAndSplashFallback`, `TextButton_SplashColor_RemainsActivationTint_AfterPointerUp`, `TextButton_StyleFrom_ForegroundOnly_DisabledFallsBackToThemeDisabledForeground`, and `ElevatedButton_StyleFrom_BackgroundOnly_DisabledFallsBackToThemeDisabledBackground` (`src/Flutter.Material/ButtonStyle.cs`, `src/Flutter.Material/Buttons.cs`, `src/Flutter.Tests/MaterialButtonsTests.cs`).
