...
- M2 focus scope progression: added `FocusScopeNode` and `FocusScope` widget wiring so focus nodes register to nearest scope and traversal remains bounded to the active scope (Tab + basic directional keys), with regression coverage for scope-local forward/reverse boundaries and directional key flow (`src/Flutter/Widgets/Focus.cs`, `src/Flutter.Tests/FocusTests.cs`).
