...
| Render object parity | `src/Flutter.Tests/RenderingParityTests.cs` | Constraint normalization, relayout boundary behavior, root relayout triggers, failed-layout exception propagation with dirty-state preservation, `BoxConstraints.Tighten` clamp behavior for out-of-range requests, and no-op proxy setter invalidation guards. | Rare constraint edge cases across proxy chains. |
