...
| Basic proxy widgets | `src/Flutter.Tests/BasicWidgetProxyTests.cs` | Widget-to-render wiring and rebuild update behavior for `Opacity`, `Transform`, and `ClipRect` wrappers over existing proxy render objects. | Higher-level composition widgets (`Align/Center`, `Stack/Positioned`) and sample-level composition scenarios. |
