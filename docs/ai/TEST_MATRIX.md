...
| Render object parity | `src/Flutter.Tests/RenderingParityTests.cs` | Constraint normalization, relayout boundary behavior, root relayout triggers, and failed-layout exception propagation with dirty-state preservation. | Rare constraint edge cases across proxy chains. |
