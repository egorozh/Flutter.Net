...
- Continued `StyleFrom(...)` parity: `foregroundColor` now derives default overlay/splash state colors when explicit values are absent, and explicit `overlayColor` now drives splash fallback (including transparent highlight/splash suppression semantics).
